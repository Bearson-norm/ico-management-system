...
export default function GitHubActionsPage() {
  ...
  const fetchRuns = async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/ga/github/actions');
  ...
  const handleTriggerDeploy = async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/ga/github/actions', { method: 'POST' });
      const json = await res.json();
      if (json.success) {
    ...
    <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      <div className="page-header" style={{ marginBottom: '24px' }}>
        <div className="flex-between">
          <div>
            <h1 className="page-title" style={{ color: 'var(--ga-tx)' }}>CI/CD & GitHub Deploy GA</h1>
            <p className="page-sub" style={{ color: 'var(--ga-tx2)' }}>Pantau status deployment otomatis ke VPS via GitHub Actions</p>
          </div>
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
            ...
            {canTrigger ? (
              <button
                ...
                style={{ backgroundColor: 'var(--ga-accent)', borderColor: 'var(--ga-accent)' }}
      ...
      <div className="card">
        <div className="card-header">
          <h2 style={{ fontSize: '16px', fontWeight: 800, color: 'var(--ga-tx)' }}>Daftar Workflow Runs Terbaru</h2>
        </div>
        <div className="table-responsive">
          <table className="table" style={{ width: '100%' }}>
            <thead>
              <tr>
                <th style={{ color: 'var(--ga-tx2)' }}>Workflow Name</th>
                <th style={{ color: 'var(--ga-tx2)' }}>Status</th>
                <th style={{ color: 'var(--ga-tx2)' }}>Trigger Event</th>
                <th style={{ color: 'var(--ga-tx2)' }}>Commit Message</th>
                <th style={{ color: 'var(--ga-tx2)' }}>Triggered By</th>
                <th style={{ color: 'var(--ga-tx2)' }}>Date / Time</th>
                <th style={{ color: 'var(--ga-tx2)' }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={7} style={{ textAlign: 'center', padding: '40px', color: 'var(--ga-tx2)' }}>
                ...
                runs.map((run) => (
                  <tr key={run.id}>
                    <td>
                      <span style={{ fontWeight: 'bold', color: 'var(--ga-tx)' }}>{run.name}</span>
                      <div className="text-tiny text-muted" style={{ color: 'var(--ga-tx2)' }}>ID: {run.id}</div>
                    </td>
                    ...
                    <td style={{ maxWidth: '280px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: 'var(--ga-tx)' }}>
                      <span title={run.head_commit?.message}>
                        {run.head_commit?.message || '—'}
                      </span>
                    </td>
                    <td style={{ color: 'var(--ga-tx)' }}>{run.head_commit?.author?.name || 'GitHub'}</td>
                    <td style={{ color: 'var(--ga-tx)' }}>
                    ...
                    <td>
                      <Link
                        ...
                        style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', color: 'var(--ga-accent)' }}
                      >
                        Lihat Log ↗
                      </Link>
                    </td>
