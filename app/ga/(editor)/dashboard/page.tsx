    ...
    <>
      ...
      <div className="page-body">
        ...
        <div className="card" style={{
          ...
          <Link href="/ga/stock" className="ga-valuation-text-link">
          ...
          <div style={{
            ...
            <Link href="/ga/database" className="ga-stat-item-link">
              <div style={{ fontSize: '10px', color: 'var(--ga-tx3)', textTransform: 'uppercase', fontWeight: 'bold' }}>Jenis Barang</div>
              <div style={{ fontSize: '18px', fontWeight: '800', color: 'var(--ga-tx)', marginTop: '2px' }}>{data?.totalItemsCount}</div>
            </Link>
            <div style={{ width: '1px', background: 'var(--ga-br)', alignSelf: 'stretch' }} />
            <Link href="/ga/stock" className="ga-stat-item-link">
              <div style={{ fontSize: '10px', color: 'var(--ga-tx3)', textTransform: 'uppercase', fontWeight: 'bold' }}>Total Fisik</div>
              <div style={{ fontSize: '18px', fontWeight: '800', color: 'var(--ga-tx)', marginTop: '2px' }}>{data?.totalStockCount?.toLocaleString('id-ID')} <span style={{ fontSize: '11px', fontWeight: '400', color: 'var(--ga-tx2)' }}>pcs</span></div>
            </Link>
            <div style={{ width: '1px', background: 'var(--ga-br)', alignSelf: 'stretch' }} />
            <Link href="/ga/database" className="ga-stat-item-link">
              <div style={{ fontSize: '10px', color: 'var(--ga-tx3)', textTransform: 'uppercase', fontWeight: 'bold' }}>Kategori</div>
              <div style={{ fontSize: '18px', fontWeight: '800', color: 'var(--ga-tx)', marginTop: '2px' }}>{data?.totalKategoriCount}</div>
            </Link>
            <div style={{ width: '1px', background: 'var(--ga-br)', alignSelf: 'stretch' }} />
            <Link href="/ga/opname" className="ga-stat-item-link">
              <div style={{ fontSize: '10px', color: 'var(--ga-tx3)', textTransform: 'uppercase', fontWeight: 'bold' }}>Opname Sesi</div>
              <div style={{ fontSize: '18px', fontWeight: '800', color: 'var(--ga-tx)', marginTop: '2px' }}>{data?.totalDraftOpnameCount} <span style={{ fontSize: '10px', color: 'var(--ga-ylw)', fontWeight: 'bold' }}>Draft</span></div>
            </Link>
            <div style={{ width: '1px', background: 'var(--ga-br)', alignSelf: 'stretch' }} />
            <Link href="/ga/po-pr" className="ga-stat-item-link">
              <div style={{ fontSize: '10px', color: 'var(--ga-tx3)', textTransform: 'uppercase', fontWeight: 'bold' }}>Barang Dipesan</div>
              <div style={{ fontSize: '18px', fontWeight: '800', color: 'var(--ga-tx)', marginTop: '2px' }}>
                {data?.activeOrderCount ?? 0} <span style={{ fontSize: '10px', color: 'var(--ga-accent)', fontWeight: 'bold' }}>Aktif</span>
              </div>
            </Link>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: '20px', alignItems: 'start' }}>
          ...
          {/* LEFT COLUMN: LOG PERGERAKAN STOK */}
          <div className="card" style={{ display: 'flex', flexDirection: 'column' }}>
            <div className="card-header" style={{ justifyContent: 'space-between', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
              <div className="card-title" style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <span>🧾</span> Log Riwayat Stok Terkini
                <select
                  value={days}
              ...
              <Link href="/ga/history" className="btn btn-ghost btn-sm" style={{ fontSize: '11px' }}>
                Lihat Semua →
              </Link>
            </div>
            <div style={{
                ...
                <table className="table-clean" style={{ width: '100%' }}>
                  ...
                  <tbody>
                    {data?.recentMovements?.map((m: any) => (
                      <tr key={m.id}>
                        ...
                        <td style={{ padding: '10px 8px' }}>
                          {m.tipe === 'IN' && <span className="badge badge-grn" style={{ padding: '2px 8px', fontSize: '10px' }}>IN</span>}
                          {m.tipe === 'OUT' && <span className="badge badge-red" style={{ padding: '2px 8px', fontSize: '10px' }}>OUT</span>}
                          {m.tipe === 'ADJ' && <span className="badge badge-blu" style={{ padding: '2px 8px', fontSize: '10px' }}>ADJ</span>}
                        </td>
                        ...
                        <td style={{ fontWeight: 700, textAlign: 'right', padding: '10px 8px', color: m.tipe === 'IN' ? 'var(--ga-grn)' : m.tipe === 'OUT' ? 'var(--ga-red)' : 'var(--ga-accent)' }}>
                          {m.tipe === 'ADJ' && m.qty > 0 ? '+' : ''}
                          {m.qty}
                        </td>
                    ...
                    {(!data?.recentMovements || data.recentMovements.length === 0) && (
                      <tr>
                        <td colSpan={5} style={{ textAlign: 'center', padding: '24px', color: 'var(--ga-tx3)' }}>
                          Belum ada transaksi log dalam {days === 1 ? 'hari ini' : days === 7 ? '1 minggu terakhir' : '30 hari terakhir'}.
                        </td>
                      </tr>
                    )}
                  </tbody>
          ...
          {/* RIGHT COLUMN: PERLU RESTOCK & OPNAME */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            
            {/* CARD 1: BARANG KRITIS & RESTOCK */}
            <div className="card">
              <div className="card-header" style={{ justifyContent: 'space-between', display: 'flex', alignItems: 'center' }}>
                <div className="card-title" style={{ color: 'var(--ga-red)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>🚨</span> Kritis / Perlu Restock
                </div>
                <Link href="/ga/stock" className="btn btn-ghost btn-sm" style={{ fontSize: '11px', color: 'var(--ga-red)' }}>
                  Lihat Stok →
                </Link>
              </div>
              <div style={{ padding: '16px 20px 20px 20px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{
                          ...
                          <div style={{ fontSize: '10px', color: 'var(--ga-tx3)', marginTop: '4px', fontWeight: '500' }}>
                            Min: {sp.minQty} {sp.uom}
                          </div>
                        </div>
                      </Link>
                    );
                  ...
                  {(!data?.lowStockItems || data.lowStockItems.length === 0) && (
                    <div style={{ textAlign: 'center', padding: '20px 0', color: 'var(--ga-tx3)' }}>
                      <span style={{ fontSize: '24px' }}>🟢</span>
                      <div style={{ fontSize: '12px', marginTop: '4px' }}>Seluruh persediaan aman.</div>
                    </div>
                  )}
                </div>
              </div>
