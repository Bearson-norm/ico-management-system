    ...
    await prisma.$transaction(async (tx) => {
      for (const row of dataRows) {
        ...
        if (trackingItem) {
          ...
          // Protect tanggalList from being overwritten by Google Sheets if PR or PO number exists
        ...
        // Auto-promote to READY_ODOO if price is set (> 0) and status is draft/initial
        ...
        const targetSpId = sparepartId || (trackingItem ? trackingItem.sparepartId : null);
        if (targetSpId) {
