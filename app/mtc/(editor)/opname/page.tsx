...
export default function MtcOpnameEditorListPage() {
  ...
  return (
    <>
      <div className="page-header flex-between">
        <div>
          <div className="page-title">📋 Stock Opname MTC</div>
          <div className="page-sub">Kelola sesi audit fisik gudang, hitung stok sparepart, dan ACC posting hasil opname</div>
        </div>
        <button
          onClick={() => setShowCreateModal(true)}
          className="btn btn-pur btn-lg"
          style={{ gap: 8 }}
        >
          <span>➕</span> Sesi Opname Baru
        </button>
      </div>
      ...
      <div className="page-body">
        ...
        <div style={{
          ...
          <div style={{ background: 'var(--sf2)', border: '1px solid var(--br)', padding: 16, borderRadius: 12, borderLeft: '4px solid #eab308' }}>
            <div style={{ fontSize: 10, color: 'var(--tx3)', fontWeight: 700, textTransform: 'uppercase' }}>Draft (Proses Hitung)</div>
          ...
          <div style={{ background: 'var(--sf2)', border: '1px solid var(--br)', padding: 16, borderRadius: 12, borderLeft: '4px solid #a855f7' }}>
            <div style={{ fontSize: 10, color: 'var(--tx3)', fontWeight: 700, textTransform: 'uppercase' }}>Menunggu ACC Manager</div>
          ...
          <div style={{ background: 'var(--sf2)', border: '1px solid var(--br)', padding: 16, borderRadius: 12, borderLeft: '4px solid #22c55e' }}>
            <div style={{ fontSize: 10, color: 'var(--tx3)', fontWeight: 700, textTransform: 'uppercase' }}>Ter-posting (Selesai)</div>
          ...
          <div style={{ background: 'var(--sf2)', border: '1px solid var(--br)', padding: 16, borderRadius: 12, borderLeft: '4px solid #3b82f6' }}>
            <div style={{ fontSize: 10, color: 'var(--tx3)', fontWeight: 700, textTransform: 'uppercase' }}>Total Seluruh Audit</div>
            <div style={{ fontSize: 22, fontWeight: 900, color: 'var(--tx1)', marginTop: 2 }}>{sessions.length} <span style={{ fontSize: 11, fontWeight: 500 }}>Sesi</span></div>
          </div>
        </div>

        {/* Sessions List */}
        <div style={{ background: 'var(--sf2)', border: '1px solid var(--br)', padding: 20, borderRadius: 16 }}>
          ...
          {loading ? (
            ...
            <div style={{ padding: 40, textAlign: 'center', border: '2px dashed var(--br)', borderRadius: 12 }}>
      ...
      {showCreateModal && (
        <div style={{
          ...
          <div style={{ background: 'var(--sf1)', border: '1px solid var(--br)', width: '100%', maxWidth: 500, padding: 24, borderRadius: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: 'var(--tx1)' }}>➕ Buat Sesi Stock Opname Baru</h3>
              <button
                onClick={() => setShowCreateModal(false)}
                style={{ background: 'none', border: 'none', color: 'var(--tx3)', fontSize: 20, cursor: 'pointer' }}
              >
                ✕
              </button>
            </div>
            ...
            <form onSubmit={handleCreateSession}>
              <div style={{ marginBottom: 16 }}>
                <label style={{ fontWeight: 700, fontSize: 12, color: 'var(--tx2)', display: 'block', marginBottom: 6 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                <button
                  type="button"
                  onClick={() => setShowCreateModal(false)}
                  style={{ padding: '8px 16px', background: 'transparent', color: 'var(--tx3)', border: 'none', cursor: 'pointer' }}
                  disabled={submitting}
                >
                  Batal
                </button>
