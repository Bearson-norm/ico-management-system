...
async function main() {
  const items = await prisma.procurementTracking.findMany({
    where: { OR: [{ nomorPr: 'PR00948' }, { nomorPr: 'PR00198' }] }
  });
  items.forEach(i => console.log(JSON.stringify({ id: i.id, pr: i.nomorPr, po: i.nomorPo, name: i.originalName, statusPr: i.statusPr, statusPo: i.statusPo, vendor: i.vendor })));
}
